...
class JPShippingRate
  ...
  private
    ...
    def load_shipping_rates
      YAML.load_file(Rails.root.to_s << "/config/rates.yml")
    end

    # Load data of region of countries
    def load_regions
      YAML.load_file(Rails.root.to_s << "/config/countries.yml")
    end

    # Load Japan areas
    def load_domestic_areas
      YAML.load_file(Rails.root.to_s << "/config/japan_areas.yml")
    end
end